...
# from vlnce_baselines.models.r2rce_prompt import make_r2rce_json_prompts
# from vlnce_baselines.models.changes import extract_waypoint_number

from gpt.api import gpt_infer
from gpt.gpt_agent import GPTNavAgent
...
class RLTrainer(BaseVLNCETrainer):
    ...
    def rollout(self, mode, ml_weight=None, sample_ratio=None):
        ...
        for stepk in range(self.max_len):
            ...
            # # llm_plan(只处理一个episode)
            # nav_input = make_r2rce_json_prompts(wp_outputs['cand_img'], t, initial_instruction=current_episodes[0].instruction.instruction_text, history=history)
            # nav_output, total_tokens = gpt_infer(nav_input, response_format={"type": "json_object"})
            # json_output = json.loads(nav_output)
            # # 构建 history 对象
            # print("json_output:", json_output, " token:", total_tokens)
            # history = {
            #     "completed": json_output.get("Completed"),
            #     "next_steps": json_output.get("Next Steps")}                    
            # waypoint_number = extract_waypoint_number(json_output, candidate_lengths[0])
